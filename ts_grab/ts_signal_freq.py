...
def plot_combined(voltage, count, fs, spectrogram_data, target_freq, span,
    ...
    # 时间-电压波形
    t_axis = np.arange(0, len(voltage), n_plot) / fs
    ax_wave.plot(t_axis, voltage[::n_plot], linewidth=0.5)
...
if __name__ == "__main__":
    # 文件路径
    bin_file = r"E:\system\data\1550\ACTS1000_data_100Msps_1s_10kS_20260821_20260821135423361_0.csv"
    ...
    try:
        ...
        channels = result.get('channels', [f'CH{i+1}' for i in range(voltage.shape[1])])
        ...
        spectrogram_data = calculate_spectrogram(
            data_to_analyze,
            fs=fs,
            time_block_duration=TIME_BLOCK,  # 转换为秒
